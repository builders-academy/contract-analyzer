...
class SmartContractAnalysisTasks:
    ...
    def diagram_task(self, agent, contract_functions):
        return Task(
            description=dedent(f"""
                Create mermaidjs diagram code showing control flow within the contract and between contracts using the following contract functions:
                {contract_functions} 
    ...
    def security_task(self, agent, contract_functions):
        return Task(
            description=dedent(f"""
                Identify and explain any potential security vulnerabilities in the code 
                by checking the functions:
                {contract_functions}
    ...
    def compiler_task(self, agent):
        return Task(
            description=dedent(f"""
                ...
                3. Diagrams (Access this from the shared memory with the key 'diagram')
